...
CPackageLocales::CPackageLocales(void)
{
}

CPackageLocales::CPackageLocales(std::wstring localeList)
{
	size_t len = localeList.length() + 1;
	wchar_t* cpy = new wchar_t[len];
	wcscpy(cpy,localeList.c_str());

	wchar_t* tok = wcstok(cpy,L",");
	while (tok != NULL)
	{
		push_back(tok);
		tok = wcstok(NULL,L",");
