...
CPackageResource::CPackageResource(std::wstring path) : m_path(path)
{
}

CPackageResource::~CPackageResource(void)
{
}

bool CPackageResource::GetFile(std::wstring path,std::wstring& contentsOut)
{
	unsigned char* pContents = NULL;
	unsigned long contentLen = 0;
	bool gotIt = GetFile(path,&pContents,&contentLen);
	if (gotIt)
	{
		wchar_t* conv = new wchar_t[contentLen + 1];
		for (unsigned long idx = 0; idx < contentLen; idx++)
		{
			conv[idx] = (wchar_t)pContents[idx];
		}
