...
module.exports = function (myCommonPaths, myStorageInfo) {
    ...
    storageInfo = extend({Version:0, Map:{}}, myStorageInfo);
...
//        }
        newSettings = extend(newSettings, oldSettings, true);
 ...
 * @param to
 * @param from
 * @param [onlyIfExists=false]
 * @return {*}
 */
function extend(to, from, onlyIfExists) {
    to = to || {};
    from = from || {};
    onlyIfExists = onlyIfExists || false;
    for (var property in from) {
        if (onlyIfExists && to[property] === undefined) continue;
