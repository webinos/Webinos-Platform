...
var AddPzp = function (_parent) {
    ...
    this.addNewPZPCert = function (_msgRcvd, _callback) {
        try {
            ...
            _parent.pzh_state.expecting.isExpectedCode (_msgRcvd.code, function (expected) { // Check QRCode if it is valid ..
                if (expected) {
                    _parent.config.generateSignedCertificate (_msgRcvd.csr, 2, function (status, value) { // Sign certificate based on received csr from client.// pzp = 2
