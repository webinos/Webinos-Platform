...
function removed(path, value) {
  if (isObjectOrArray(value)) {
    ...
    return extend(_detectUpdates(path, value, {}),
                  [new Command('remove', path, value)]);
  }

  /* primitive removed */
  return [new Command('remove', path, value)];
}
...
exports.reconcile = function(commandLists) {
  var propagations = [];
  var conflicts = [];
 ...
 **/
exports.applyCommands = function(target, commands) {
