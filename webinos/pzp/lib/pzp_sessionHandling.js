...
var Pzp = function () {
  ...
  function pzpClient_Authorized(client, msg) {
    ...
    logger.log("authorized & connected to PZP: " + msg.address + " name  " + peerSessionId);
    ...
    session.common.processedMsg(self, msgObj, function(validMsgObj) {
      ...
      if (validMsgObj.type === 'prop') {
        switch(validMsgObj.payload.status) {
          ...
          break;
          case "findServices":
          prepMsg(sessionId, validMsgObj.from, "foundServices", {"message": discovery.getAllServices(validMsgObj.from), id: validMsgObj.payload.message.id});
          break;
   ...
   */
  function connectHub(code, callback) {
    ...
    try {
      setConnParam(function(options){
        ...
        pzpClient.on("error", function (err) {
          ...
          logger.error("error connecting server due to " + err.message);
        });
      });
    } catch (err) {
      ...
      setConnParam(function(certConfig) {
        ...
        tlsServer = tls.createServer(certConfig, function (conn) {
          ...
          conn.on("end", function () {
            logger.log("pzp server - connection end with pzp client " + conn.id );
            cleanUp(conn.id);
          });
  ...
  this.connectPeer = function (msg) {
    setConnParam(function(options){
      var client = tls.connect(msg.port, msg.address, options, function () {
        if (client.authorized) {
          pzpClient_Authorized(client, msg);
        } else {
          logger.error("pzp client - connection failed, first connect with PZH ");
        }
      });
      ...
      if(status){
        ...
        try {
          ...
          function(status, value){
            if (status) {
              connectHub(inputConfig.code, function(status, value) {  // connects hub
                if (status){
                  logger.log("connected to PZH successful ");
                } else {
