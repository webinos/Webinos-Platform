...
exports.startPzpWebSocketServer = function(pzpInstance, config, callback) {
	...
	function wsMessage(connection, utf8Data) {
		...
		if(msg.type === 'prop' ) {
			if(msg.payload.status === 'disconnectPzp') {
				if( typeof pzpInstance !== "undefined" && typeof pzpInstance.sessionId !== "undefined") {
					if(pzpInstance.connectedPzp.hasOwnProperty(pzpInstance.sessionId)) {
						pzpInstance.connectedPzp[pzpInstance.sessionId].socket.end();
						pzpInstance.wsServerMsg("Pzp "+pzpInstance.sessionId+" closed");
					}
				}
			}
			...
			if( typeof pzpInstance !== "undefined" && typeof pzpInstance.sessionId !== "undefined") {
				pzpInstance.messageHandler.onMessageReceived(msg, msg.to);
			}
		}
	}
	...
	if(wrtServer) {
		wrtServer.listener = function(connection) {
			...
			pzpInstance.connectedApp(connection);
