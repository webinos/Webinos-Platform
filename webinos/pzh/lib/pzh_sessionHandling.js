...
var Pzh = function () {
  ...
  var modules;         // holds startup modules
  var listenerMap= {}; // holds listeners/callbacks, mostly for pzh internal api
  var self = this;
  ...
  function initializeRPC(){
    ...
    registry.loadModules(config.serviceCache, rpcHandler); // load specified modules
  ...
  function handlePzhAuthorization(pzhId, conn) {
    ...
    if (!connectedPzh.hasOwnProperty(pzhId)) {
      ...
      var localServices = discovery.getAllServices();
    ...
    setOptions(function(status, options) {
      for (key = 0; key <  config.trustedList.pzh.length; key = key + 1) {
        if (!connectedPzh.hasOwnProperty(config.trustedList.pzh[key])) {
          self.connectOtherPZH(config.trustedList.pzh[key], options, config.userPref.ports.provider,
  ...
  this.getConnectedPzp = function() {
    ...
    for (myKey=0; myKey < config.trustedList.pzp.length; myKey = myKey + 1){
      if (connectedPzp.hasOwnProperty(config.trustedList.pzp[myKey])){
        pzps.push({id: config.trustedList.pzp[myKey], url: sessionId + "/"+config.trustedList.pzp[myKey], isConnected: true});
      } else {
        pzps.push({id: config.trustedList.pzp[myKey], url: sessionId + "/"+config.trustedList.pzp[myKey], isConnected: false});
      }
    }
    return pzps;
  };
  this.getConnectedPzh = function() {
    ...
    for (myKey=0; myKey < config.trustedList.pzh.length; myKey = myKey + 1){
      if (connectedPzh.hasOwnProperty(config.trustedList.pzh[myKey])){
        pzhs.push({id: config.trustedList.pzh[myKey], url: config.trustedList.pzh[myKey], isConnected: true});
      } else {
        pzhs.push({id: config.trustedList.pzh[myKey], url: config.trustedList.pzh[myKey], isConnected: false});
      }
    }
    return pzhs;
  };
  ...
  this.revokedCert = function() {
    var myKey, revokedCert = [];
    for (myKey in config.cert.internal.revokedCert){
      revokedCert.push({id: myKey})
    }
    return revokedCert;
  };
  ...
  this.handleConnectionAuthorization = function(conn) {
    if(conn.authorized === false) {// Allows PZP to connect if it has proper QRCode
      logger.log(" connection NOT authorised at pzh");
      self.expecting.isExpected(function(expected) {//If this is a new PZP, we allow if it has proper QRCode
        if (!expected || conn.authorizationError !== "UNABLE_TO_GET_CRL" ){//we"re not expecting anything - disallow.
          logger.log("ending connect: " + conn.authorizationError);
          conn.socket.end();
        } else {
          logger.log("continuing connect - expected: " + conn.authorizationError);
        }
      });
    }
  ...
  /**
   * @description: Adds new PZP certificate. This is triggered by client, which sends its csr certificate and PZH signs
  ...
  /**
   * @description: ADDs PZH in a provider
   * @param {string} uri: pzh url you want to add .. assumption it is of form pzh.webinos.org/nick
   * @param {object} user: modules that will be supported on PZH
  ...
  this.addOtherZoneCert = function(from, sendCertificate, callback) {
