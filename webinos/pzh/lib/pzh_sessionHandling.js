...
Pzh.prototype.sendMessage = function (message, address, conn) {
	var buf, self = this;
	try {
		var tag = parseInt(session.configuration.PZH_MSG, 2);
		var length;
		...
		log(self.sessionId, "INFO", "[PZH -"+ self.sessionId+"] Send to "+ address /*+ " Message "+JSON.stringify(messageSend)*/);
		buf = new Buffer(JSON.stringify(messageSend), "utf8");
	...
	if (typeof farm.server === "undefined" || farm.server === null) {
		...
		if (typeof uri === "undefined" || uri === "null" || typeof modules === "undefined" || modules === "null" ){
			...
			session.configuration.setConfiguration(name, "Pzh", uri, function(config, conn_key) {
				...
				instance.sessionId = name ;//Pzh_DeviceName
